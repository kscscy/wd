...
@Controller
@RequestMapping("/test09/*") // 확장자를 붙이지 않는 것이 유지보수에 좋다.
public class Test09 {
	...
	public ModelAndView e1() throws Exception {
		...
		// JSP에 전달할 값을 저장한다.
		// 프런트 컨트롤러는 이 객체에 저장된 값을 꺼내 HttpServletRequest객체에 옮길 것이다.
	 ...
	 * HttpEntity or ResponseEntitiy에 담아서 리턴하라
	 */
	@RequestMapping("e6")
	public ResponseEntity<String> e6() throws Exception {
		...
		ResponseEntity<String> responseEntity = new ResponseEntity<String>(
				...
				headers,						// 추가할 응답 헤더
				HttpStatus.CREATED	// 응답 상태 코드 created(201)
				);
