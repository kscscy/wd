...
public class Test03 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // log4j.rootCategory=출력 등급, 출력 담당자 이름
    // ex) log4j.rootCategory=TRACE, xxxstdoutxx(마음대로 상관없다)
    //
    // 출력 담당자 이름
    // => 마음대로 짓는다.
    //
    // 2) 출력 담당자의 유형을 설정하기
    // log4j.appender.xxxstdoutxx(위에 이름과 일치)=출력 담당자의 유형
    // ex) log4j.appender.stdout=org.apache.log4j.ConsoleAppender
    // 
    // 출력 담당자의 유형?
    ...
    // ex) log4j.appender.stdout.layout=org.apache.log4j.PatternLayout
    //
    // 출력 형식?
    // SimpleLayout     : 단순한 출력
    ...
    // ex) log4j.appender.stdout.layout.ConversionPattern=%d{ABSOLUTE} %5p %t %c{2}:%L - %m%n
    //
    // 패턴?
    // %패턴명령
    // ex) %5p %t
    ...
    // %n     : 줄 바꿈
    // %p     : 로깅 등급 이름
    // %5p    : 출력 칸 수 지정=> 5문자 -5p: 앞에서부터
    // %m     : 로그 메시지
    // %d{HH:mm:ss yyyy-MM-dd} : 날짜 및 시간 출력, 날짜 및 시간 형식은 java.text.SimpleDateFormat클래스참조
    // %t     : 스레드 이름
